...
class Bing(object):
    ...
    def __init__(self, *args, **kwargs):
        return super(Bing, self).__init__(*args, **kwargs)
    ...
    def bing_query(keyword):
        ...
        for sn, sz in enumerate(['medium', 'large', 'wallpaper'][::-1]):
            ...
            while True:
                ...
                start = time.clock()
                response = requests.get(Bing.BING_REST, headers=Utils.HEADERS, params=params, timeout=Utils.QUERY_TIMEOUT)
                if response:

                    if response.status_code == 200:
                        sys.stdout.write('\t Done. [SUCC] (%.2fs)\n'%(time.clock() - start))
                        #print(response.content.decode('utf-8','ignore'))
                        urls = re.findall(Bing.BING_PARN, response.content.decode('utf-8','ignore'))
